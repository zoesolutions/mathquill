...
_.html_template = ['<span class="array matrix"></span>', '<span class="normal"></span>'];
_.latex = function() {
	return '\\begin{pmatrix}' + this.foldChildren([], function(latex, child) {
		var childLatex = child.latex();
		if (latex.length > 0 && child.jQ.is('.normal')) {
			latex[latex.length - 1] = latex[latex.length - 1] + '&' + childLatex;
	    } else {
	    	latex.push(childLatex);
	    }
...
_.keydown = function(e) {
	...
	if (currentBlock.parent === this) {
		if (e.which === 13) { //enter
			...
			newBlock.jQ = $('<span class="break"></span>')
			...
			var newBlock = new MathBlock;
			newBlock.parent = this;
			newBlock.jQ = $('<span class="normal"></span>').data(jQueryDataKey, {block: newBlock}).appendTo(this.jQ);
			this.lastChild = newBlock;
			currentBlock.next = newBlock;
			newBlock.prev = currentBlock;
			this.cursor.appendTo(newBlock).redraw();
			...
			var newBlock = new MathBlock;
			newBlock.parent = this;
			newBlock.jQ = $('<span class="break"></span>').data(jQueryDataKey, {block: newBlock}).appendTo(this.jQ);
			this.lastChild = newBlock;
			currentBlock.next = newBlock;
			newBlock.prev = currentBlock;
			this.cursor.appendTo(newBlock).redraw();
